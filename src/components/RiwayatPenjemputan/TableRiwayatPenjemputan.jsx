...
export default function CustomTableRiwayat() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get("https://admin-api.ewhale.my.id/api/riwayat-penjemputan");
    ...
    const handleDetailClick = async (pickupId) => {
        try {
            const response = await axios.get(`https://admin-api.ewhale.my.id/api/riwayat-penjemputan/${pickupId}`);
            if (response.data.success) {
